...
```std::promise``` 是 C++11 引入的线程间同步通信工具，核心作用是**在一个线程中设置结果/异常，在另一个线程中获取该结果/异常**，常与```std::future```
配对使用（```promise```负责“生产”结果，```future```负责“消费”结果）。
...
int main() {
    ...
    std::thread t(calculate_sum, std::move(sum_prom), 10, 20);    # promise 必须通过移动语义传递，无法拷贝

    std::cout << "主线程等待子线程计算结果..." << std::endl;

    int result = sum_fut.get();
